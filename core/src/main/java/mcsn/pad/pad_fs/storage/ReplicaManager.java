...
public class ReplicaManager extends Thread {
	...
	public void run() {
		...
		while (isRunning.get()) {
			try {
				Thread.sleep(delta);
				List<Member> view = membershipService.getMembers();
				if (view.size() > 0) {
					Member partner = view.get( random.nextInt( view.size()) );
					InetSocketAddress raddr = new InetSocketAddress(partner.host, storagePort);
